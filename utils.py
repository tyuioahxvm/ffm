...
def split_data(source_path, train_path, test_path, prob):
    with open(train_path,'wb') as fr:
        with open(test_path,'wb') as fe:
...
def split_csv(path, nr_thread, has_header):
    def calc_nr_lines_per_thread():
        nr_lines = int(
            list(subprocess.Popen('sudo wc -l {0}'.format(path), shell=True, stdout=subprocess.PIPE).stdout)[0].split()[
                0])
        if not has_header:
            nr_lines += 1
